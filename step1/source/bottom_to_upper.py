...
class BottomToUpper(InMemoryDataset):
    url = ('')
    ...
    def __init__(self, root: str, transform: Optional[Callable] = None,
        ...
        print(f"\x1b[32m\"하의류(스타일, 중분류, 성별) 클래스 ➡ 상의아이템 추천 datasets 구축.\"\x1b[0m")
    @property
    def raw_file_names(self) -> List[str]:
        return ['bottom_class_list.txt', 'upper_item_list.txt', 'train.txt', 'test.txt']
    ...
    def process(self):
        ...
        data = HeteroData()
        print(f"\x1b[32m\"하의류(스타일, 중분류, 성별) 클래스 ➡ 상의아이템 추천 datasets 구축. {data}\"\x1b[0m")
        
        # Process number of nodes for each node type:
        # 각 노드 유형에 대한 프로세스 노드 수
        node_types = ['bottom', 'upper']
        ...
        for path, attr_name in zip(self.raw_paths[2:], attr_names):
            ...
            data['bottom', 'rates', 'upper'][attr_name] = index
            if attr_name == 'edge_index':
                data['upper', 'rated_by', 'bottom'][attr_name] = index.flip([0])
